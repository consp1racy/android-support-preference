...
public abstract class GenericInflater<T, P> {
    ...
    private void rInflate(XmlPullParser parser, T parent, final AttributeSet attrs)
        ...
        while (((type = parser.next()) != XmlPullParser.END_TAG ||
            ...
            if (parent instanceof Parent) {
                ((Parent<T>) parent).addItemFromInflater(item);
            } else {
                reflectAddItemFromInflater(parent, item);
            }
    ...
    private static <T> void reflectAddItemFromInflater(Object parent, T item) {
        Method m = getAddItemFromInflaterMethod(parent.getClass(), item.getClass());
        try {
            m.invoke(parent, item);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static Method getAddItemFromInflaterMethod(Class cls, Class itemCls) {
        Method m = METHODS_ADD_ITEM_FROM_INFLATER.get(cls);
        if (m == null) {
            ...
            METHODS_ADD_ITEM_FROM_INFLATER.put(cls, m);
        }
        return m;
    }
